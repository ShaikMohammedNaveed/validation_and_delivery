...
class NotebookValidator:
    def __init__(self, content: str, file_path: str):
        ...
        self.error_file = f"{settings.LWC_OUTPUT_DIR}/error/{file_path}_error.txt"
    ...
    def validate_conversation(self):
        ...
        # print("**Conversation**" in self.sections.keys())
        ...
        #         if i > 3 and raw_lines[i-2].strip() == "" and raw_lines[i-2].strip() in ['**Assistant**', '**Implementation plan**', '**Scaffolding code**', '**Code**']:
        #             self.errors.append(
                    ...
                    for subheading_name, subheading_regex in subheading_order:
                        if subheading_regex.search(text):
                            ...
                            if subheading_index == 0 and subheading_name != "Blueprint":
                                ...
                                self.errors.append(f"❌ Cell {cell_num + 2}: Expected '**{subheading_order[subheading_index % 4][0]}**' but found '**{subheading_name}**' out of order.")
                            subheading_index += 1
    ...
    def validate_blueprint(self, text, cell_num):
        ...
        for raw_line in lines:
            ...
            if len(line_stripped) < 15 and line_stripped.lower() in ['category', 'screenshot', 'problem statement','message','complexity category','tags category','subcategory']:
                self.errors.append(
            ...
            for idx_local, name_line_idx in enumerate(name_line_indices):
                ...
                while i_sub < len(sub_block):
                    ...
                    if match_opt_bold:
                        ...
                        # print(field_name, field_name in plurals_to_singulars)
                ...
                # REMOVED: We no longer append the missing Step error to avoid duplicates
                # if not has_step:
                #     self.errors.append(
                #         f"❌ Cell {cell_num}: Missing 'Step' entry after 'Name' line: '{name_line_stripped}'."
                #     )
    ...
    # ):
    #     """
    #     A helper function that does the line-by-line validation:
    #     - Looks for lines matching `file_line_regex` (filename in backticks).
    #     - Requires an immediately following code fence (```lang).
    #     - Checks the declared language against the file extension in `code_block_type_map`.
    #     - Checks for a closing ``` and logs errors if missing.

    #     :param lines: List of text lines to validate
    #     :param cell_num: Notebook cell number (for error context)
    #     :param file_line_regex: Regex to match the file line (including or excluding .scaf)
    #     :param code_fence_start_regex: Regex to match the code fence start line (```lang)
    #     :param code_fence_end: The string (typically "```") that ends a code fence
    #     :param code_block_type_map: Dict mapping extensions -> list of valid languages
    #     :param scaf_required: If True, enforces that the filename must end with ".scaf"
    #     """
    #     i = 0
    #     n = len(lines)
    ...
    #         if file_match:
    #             # e.g. "OpportunityManagerBasicController.cls.scaf"
    #             full_file_name = file_match.group(1)
    #             # e.g. "cls" if matched group(2) is the extension
    #             extension = file_match.group(2).lower() if len(file_match.groups()) > 1 else ""
    #             # If scaf_required, ensure file actually ends with .scaf
    #             if scaf_required and not full_file_name.lower().endswith(".scaf"):
    #                 self.errors.append(
    #                     f"❌ Cell {cell_num}: File '{full_file_name}' does not end with '.scaf'."
    #                 )

    #             # Move to next line(s) to find a code fence
    #             j = i + 1
    #             # Skip blank lines
    ...
    #             if j >= n:
    #                 # No line left for a code fence
    #                 self.errors.append(
    #                     f"❌ Cell {cell_num}: No code fence found after file '{full_file_name}'."
    #                 )
    #                 break

    #             # Expect code fence start line: ```lang
    #             fence_line = lines[j].strip()
    #             fence_match = code_fence_start_regex.match(fence_line)
    #             if not fence_match:
    #                 self.errors.append(
    #                     f"❌ Cell {cell_num} Please remove space after ``` in {fence_line}" if ' ' in fence_line else f"❌ Cell {cell_num}: Expected a code fence (```lang) after file '{full_file_name}', but found '{fence_line}'."
    #                 )
    #                 i = j
    #                 continue

    #             # We have a code fence => get declared language
    #             declared_lang = fence_match.group(1).lower()

    #             # Validate that the declared language matches extension
    #             valid_langs = code_block_type_map.get(extension, [])
    #             if not valid_langs:
    #                 # extension not recognized
    #                 self.errors.append(
    #                     f"❌ Cell {cell_num}: Unrecognized file extension '.{extension}' "
    #                     f"for '{full_file_name}'. Cannot validate code block language."
    #                 )
    #             else:
    ...
    #             # If found, jump i beyond the fence
    #             i = k + 1
    #         else:
    #             # If line starts with ``` but there's no preceding file line => error
    #             if line.startswith("``` "):
    #                 self.errors.append(
    #                     f"❌ Cell {cell_num}: Found a code fence with a space after the opening triple backticks. {line}"
    #                 )
...
#         # Match all headers in **Header** format dynamically
#         header_pattern = re.compile(r'\*\*(.*?)\*\*')
#         headers = header_pattern.findall(metadata_content)

#         # for header in headers:
#         #     # Extract the content after the header
#         #     content_pattern = re.compile(rf'\*\*{re.escape(header)}\*\*\s*(.*?)(?=\n\*\*|\Z)', re.DOTALL)
#         #     content_match = content_pattern.search(metadata_content)

#         #     if content_match:
#         #         content = content_match.group(1).strip()

#         #         # Check if the content only consists of four asterisks (indicating an empty header)
#         #         if content.count('*') == 4:
#         #             self.errors.append(f"Missing content for '**{header}**'.")

#         #     else:
#         #         self.errors.append(f"Missing content for '**{header}**'.")

#         # Special case: Validate **manualSetupRequired** must be True or False
#         manual_pattern = re.compile(r'\*\*manualSetupRequired\*\*\s*-\s*(\S+)')
#         manual_match = manual_pattern.search(metadata_content)
#         if manual_match:
#             manual_value = manual_match.group(1).strip()
#             if manual_value not in ["True", "False"]:
...
#         clarification_pattern = re.compile(r'\*\*Clarification Question\*\*')
...
#         # Track conversation blocks
#         blocks = []
#         current_role = None
#         current_text = []
#         cell_number = 1  # Track cell number
...
#             if not name_line_indices:
#                 self.errors.append(
#                     f"❌ Cell {cell_num}: Section '**{header_text}**' does not contain any 'Name' entries."
#                 )
#                 # move to next header
#                 continue
...
#                 if not fence_match:
#                     self.errors.append(
#                         f"❌ Cell {cell_num}: Expected a code fence (```lang) after file '{full_file_name}', "
#                         f"but found '{fence_line}'."
#                     )
#                     i = j  # move forward anyway
#                     continue
...
#             error_file = f"{os.path.splitext(file_path)[0]}_error.txt"
#             if os.path.exists(error_file):
#                 with open(error_file, 'r', encoding='utf-8') as f:  # FIX: Read file with UTF-8
#                     summary_errors.append(f"{file} Errors:\n" + f.read())
    
#     summary_file = os.path.join(folder_path, "summary_errors.txt")
#     if summary_errors:
#         with open(summary_file, 'w', encoding='utf-8') as f:  # FIX: Write file with UTF-8
#             f.write("\n".join(summary_errors))
#         print("\n📜 Summary of errors saved in", summary_file)
#     elif os.path.exists(summary_file):
#         os.remove(summary_file)  # Remove old summary file if no errors remain

###############################################################################################################################
def extract_markdown_from_ipynb(nb_content: str) -> str:
    try:
        # Parse the string as JSON notebook
        nb_json = json.loads(nb_content)
        nb = nbformat.from_dict(nb_json)
    except Exception as e:
        return f"Failed to parse notebook content: {str(e)}"
    ...
    for cell in nb.cells:
        if cell["cell_type"] == "markdown":
            ...
            # If 'source' is a list of lines, join them into a single string:
            if isinstance(source_data, list):
                combined_source = "".join(source_data)
            else:
                combined_source = source_data

            markdown_cells.append(combined_source)

    return "\n\n".join(markdown_cells)



def parse_ipynb_and_extract_markdown(nb_content: str) -> (str, str):
    """
    Reads a string containing JSON for a Jupyter notebook,
    returns a tuple of (markdown_text, error_msg).
    If parsing fails, markdown_text will be "" and error_msg will be non-empty.
    """
    try:
        nb_json = json.loads(nb_content)
        nb_obj = nbformat.from_dict(nb_json)
    except Exception as e:
...
# ------------------------------------------------------------------------------
# Updated validate_notebook using the NotebookValidator class
# ------------------------------------------------------------------------------
def validate_notebook(
    ...
    """
    Processes notebooks from `input_batch` (each item has 'content' and 'metadata'),
    validates them with NotebookValidator, and writes any errors to a Google Sheet
    along with Collab links.

    Splits or truncates any error messages exceeding 50k characters to avoid
    the Google Sheets per-cell limit.

    :param input_batch: A dict like:
        {
          'items': [
             {
               'content': '<string JSON of .ipynb>',
               'metadata': {
                  'data': {
                     'file_id': 'some_file_id',
                     'original_uri': 'https://colab.research.google.com/drive/<...>'
                     ...
                  }
               }
             },
    ...
    :param CREDENTIALS_PATH: Path to your service account JSON file.
    :param INPUT_SHEET_NAME: Name (or tab) of the Google Sheet to write into.
    :param INPUT_SHEET_ID: ID of the Google Sheet (found in its URL).
    """
    import io
    from google.oauth2.service_account import Credentials
    from googleapiclient.discovery import build
    from validation.lwc_validator_reviewer import NotebookValidator  # <— import your class

    # Helper function to ensure an error text chunk is under 50,000 characters
    # We either truncate or split lines if desired
    MAX_CELL_LENGTH = 50000

    def safe_cell_content(content):
        """
        Option A (Truncation):
            If content > 50k, we cut it and append '...(truncated)'
        Option B (Splitting):
            If you want to create multiple lines, you'd do it differently.
        For now we do truncation for simplicity.
        """
        if len(content) <= MAX_CELL_LENGTH:
            return content
        else:
            return content[:MAX_CELL_LENGTH] + "...(truncated)"

    # Track issues and successful notebooks
    failed_notebooks = []
    successful_items = []

    # 1. Iterate over each item in the input batch
    for item in input_batch.get("items", []):
        file_id = item["metadata"]["data"].get("file_id", "UNKNOWN_FILE_ID")
        file_url = item["metadata"]["data"].get("original_uri", "UNKNOWN_URL")
        nb_content = item["content"]

        #print(f"\nValidating notebook with File ID: {file_id}")

        # Parse .ipynb => Extract markdown (example function from your earlier code).
        from validation.lwc_validator_reviewer import parse_ipynb_and_extract_markdown
        markdown_text, parse_error = parse_ipynb_and_extract_markdown(nb_content)
        if parse_error:
            # If the ipynb was not parseable, treat that as an error
            errors = [parse_error]
            failed_notebooks.append((file_id, file_url, errors))
            continue

        # Use the NotebookValidator class to validate
        pseudo_filepath = f"{file_id}.ipynb"
        validator = NotebookValidator(markdown_text, pseudo_filepath)
        validator.validate()
    ...
    # Keep only successful notebooks
    input_batch["items"] = successful_items

    # Connect to Google Sheets
    creds = Credentials.from_service_account_file(CREDENTIALS_PATH)
    sheets_service = build('sheets', 'v4', credentials=creds)

    # Clear old contents in columns A–C
    clear_range = f"{INPUT_SHEET_NAME}!A:C"
    sheets_service.spreadsheets().values().clear(
        spreadsheetId=INPUT_SHEET_ID,
        range=clear_range
    ).execute()
    #print("Cleared previous issue records from the Google Sheet.")
    count_of_collabs_with_issues = len(failed_notebooks)

    # Write any notebook errors
    if failed_notebooks:
        #print("\nNotebooks with issues:")
        values = [["File ID", "Collab Link", "Issues"]]
        for (fail_file_id, fail_url, errs) in failed_notebooks:
            #print(f"- {fail_file_id} => {len(errs)} error(s)")
            joined_errors = "\n".join(errs)

            # Ensure the text for 'Issues' does not exceed 50k
            joined_errors = safe_cell_content(joined_errors)

            values.append([fail_file_id, fail_url, joined_errors])

        body = {"values": values}
        range_name = f"{INPUT_SHEET_NAME}!A1"
        ...
        print("Issues have been saved to the Google Sheet.\n")
        return {"data":input_batch, "status":"failed", "count_of_collabs_with_issues":count_of_collabs_with_issues}
    else:
        print("No errors found. The Google Sheet remains empty.\n")

        return {"data":input_batch, "status":"success"}
