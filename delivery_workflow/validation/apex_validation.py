...
def load_notebook(file_path):
    ...
    try:
        ...
        with open('collab to json,json', 'w', encoding='utf-8') as f:
...
def validate_notebooks_in_input_batch(input_batch, CREDENTIALS_PATH, INPUT_SHEET_NAMES, INPUT_SHEET_ID):
    """
    Validates notebooks in the input_batch, drops failed notebooks,
    clears previous issues, saves new issues to Google Sheet,
    and returns only successfully validated notebooks.

    :param input_batch: Dictionary containing notebook metadata and content.
    :param CREDENTIALS_PATH: Path to Google API credentials file.
    :param INPUT_SHEET_NAMES: Name of the sheet/tab in Google Sheets.
    :param INPUT_SHEET_ID: Google Sheet ID where issues will be recorded.
    :return: Filtered input_batch containing only successfully validated notebooks.
    """
    # Track issues and successful notebooks
    failed_notebooks = []  # Store tuples of (file_id, URL, errors)
    successful_batch = []  # Store successful notebook metadata

    for item in input_batch['items']:
        file_id = item['metadata']['data']['file_id']
        file_url = item['metadata']['data']['original_uri']
        content = item['content']

        print(f"\n🗎 Validating notebook with File ID: {file_id}")

        # Run the notebook validation (mocked here as an example)
        notebook_type, errors = generate_validation_report(content)  # Replace with actual validation logic
    ...
    # Update input_batch to only include successful notebooks
    input_batch['items'] = successful_batch

    # Connect to Google Sheets
    creds = Credentials.from_service_account_file(CREDENTIALS_PATH)
    sheets_service = build('sheets', 'v4', credentials=creds)

    # Define the range to clear before writing new issues
    clear_range = f"{INPUT_SHEET_NAMES}!A:C"  # Clears columns A to C (File ID, URL, Issues)
    sheets_service.spreadsheets().values().clear(
        spreadsheetId=INPUT_SHEET_ID,
        range=clear_range
    ).execute()
    print("\nCleared previous issue records from the Google Sheet.")
    count_of_collabs_with_issues = len(failed_notebooks)
    # Save issues from failed notebooks to Google Sheet
    if failed_notebooks:
        print("\nThe following notebooks had issues:")
        values = [["File ID", "URL", "Issues"]]  # Header row
        for (file_id, url, errs) in failed_notebooks:
            print(f"- {file_id} => {len(errs)} error(s)")
            values.append([file_id, url, "\n".join(errs)])

        body = {'values': values}
        range_name = f"{INPUT_SHEET_NAMES}!A1"
        ...
        print("\nIssues have been saved to the Google Sheet.")
...
def generate_validation_report(content):
    """
    Runs all validation checks on the notebook content and generates a structured validation report.
    Returns (notebook_type, errors).

    :param content: Notebook content as a JSON string.
    ...
    notebook_type = detect_notebook_type(notebook_data.get('cells', []))

    if not notebook_type:
        print("Error: Could not determine notebook type.")
        return (None, ["Unknown notebook type"])

    print(f"\n📘 Detected Notebook Type: {notebook_type.upper()}")

    validation_errors = []
    issue_errors = validate_issue_count(notebook_data.get('cells', []), notebook_type)
    structure_errors = validate_notebook_structure(notebook_data.get('cells', []), notebook_type)
    content_errors = validate_content_formatting(notebook_data.get('cells', []), notebook_type)
    issue_block_errors = validate_issue_block_headers(notebook_data.get('cells', []))
...
if __name__ == "__main__":
    mode_choice = input("Choose Mode: \n1. Validate a single notebook\n2. Validate all notebooks in a folder\n\nEnter choice (1 or 2): ").strip()

    if mode_choice == "1":
        file_path = input("Enter the path to the notebook file: ").strip()
        generate_validation_report(file_path)

    elif mode_choice == "2":
        folder_path = input("Enter the path to the folder containing notebooks: ").strip()
        validate_notebooks_in_folder(folder_path)
